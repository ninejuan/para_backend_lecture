...
async def create(member: Member):
    try:
        ...
        return {"message": "부원 정보가 생성되었습니다.", "member_id": member_id}
...
async def getMember(member_id: str):
    try:
        ...
        if not member:
            raise HTTPException(status_code=404, detail="부원 정보를 찾을 수 없습니다.")
        return member
...
async def updateMember(member_id: str, member: Member):
    try:
        ...
        if not updated_id:
            raise HTTPException(status_code=404, detail="부원 정보를 찾을 수 없습니다.")
        return {"message": "부원 정보가 수정되었습니다.", "member_id": updated_id}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def deleteMember(member_id: str):
    try:
        ...
        if not deleted_id:
            raise HTTPException(status_code=404, detail="부원 정보를 찾을 수 없습니다.")
        return {"message": "부원 정보가 삭제되었습니다.", "member_id": deleted_id}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))